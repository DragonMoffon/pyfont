...
class Table(TTFType, metaclass=Definition):
    ...
    __selectors__: tuple[
        tuple[TTFVersion, Callable[[TTFVersion, TTFVersion], bool], type[Table]], ...
    ] = ()
    ...
    @classmethod
    def parse(cls: Self, record: TableRecord, font: Font, buffer: bytes) -> Self:
        cls = cls.find_version(record, font, buffer)
        return cls._parse(record, font, buffer)

    @classmethod
    def _parse(cls: Self, record: TableRecord, font: Font, buffer: bytes):
